...
elseif($_SESSION["loggedin"]==1 && (time()-$_SESSION["last"])<=1800)
{
	...
	<div id="conmax">
	<center>
	<p style="font-family: Consolas, Andale Mono, Lucida Console, Lucida Sans Typewriter, Monaco, Courier New, monospace; font-size: 24px;">&lt;&lt;Select an Item&gt;&gt;</p>
	...
	$categ=$_GET['id'];
	$sql="SELECT pid,pname,price FROM stock WHERE ptype='$categ'";
	$result=$conn->query($sql);
	$entri=$result->num_rows;
	$r=floor($entri/3);
	$c=$entri%3;
		...
		for ($i = 1; $i <= $r; $i++) {
			for ($j=1;$j<=3;$j++) {
				...
				if($categ=='electronics')
					echo '<a href="item.php?id='.$row["pid"].'"><img src="pimages/'.$row["pid"].'.jpg" width="100" height="201"></a><br>';
				else
					echo '<a href="item.php?id='.$row["pid"].'"><img src="pimages/'.$row["pid"].'.jpg" width="181" height="205"></a><br>';
				echo $row["pname"].'<br>';
				echo '<b>Rs. '.$row["price"].'/-</b>';
				echo '</center></td>';
				if($j%3==0) {
					echo '</tr>';
				}
	...
	echo '<br><a href="home.php"><img src="go-back.png" width="64" height="64"></a>
	</center>';
	echo '</div></center>';
